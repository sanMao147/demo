...
import mapPoints from '@/config/mapPoints'
...
const init = () => {
  ...
  function screenRender() {
    ...
    controls.update()
    timer = requestAnimationFrame(screenRender)
  }
  ...
  function createMapPoints() {
    ...
    for (let point of mapPoints.points) {
      ...
      if (pos.x && pos.y && pos.z) {
        // 生成点阵
        const pingGeometry = new THREE.SphereGeometry(
          0.4,
          5,
          5
        )
  ...
  function convertLatLngToSphereCoords(
    ...
    const x =
      -(radius + -1) * Math.cos(phi) * Math.cos(theta)
    const y = (radius + -1) * Math.sin(phi)
    const z =
      (radius + -1) * Math.cos(phi) * Math.sin(theta)
    return {
      x,
      y,
      z,
  ...
  const domain = [
    1000, 3000, 10000, 50000, 100000, 500000, 1000000,
    1000000,
  ]
